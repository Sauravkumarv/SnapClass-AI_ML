...
def student_dashboard():
  ...
  try:
    ...
    st.error('Could not load your subjects. Please refresh and try again.')
    footer_login()
    return
  ...
  for log in logs:
    ...
    if sid not in stats_map:
      stats_map[sid] = {"total": 0, "attendence": 0}
    ...
    def uneroll_button(sid=sid):
      if st.button("Unenroll from this course",type='tertiary',width='stretch',icon=':material/delete_forever:', key=f"unenroll_{sid}"):
        try:
          ...
          st.error('Could not unenroll. Please try again.')
...
def student_screen():
  ...
  if photo_source:
    try:
      ...
      st.error('Face scan failed. Please try again with better lighting.')
        ...
        if new_name:
          with st.spinner('Creating Profile...'):
            try:
              ...
              st.error("Something went wrong during registration. Please try again.")
        else:
